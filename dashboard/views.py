...
def home_view(request):
    ...
    if perms['can_view_all']:
        ...
        clients_qs = Client.objects.filter(assignments__assigned_to=user, assignments__is_active=True).distinct()
    ...
    for c in clients_qs:
        ...
        c.active_csm = assignment.assigned_to if assignment else None
        clients.append(c)
    
    csm_users = User.objects.filter(is_active=True).exclude(id=user.id) if perms['can_assign'] else []
    
    return render(request, 'dashboard/dashboard.html', {
        'clients': clients, 'csm_users': csm_users, 'perms': perms, 'stage_choices': Client.STAGE_CHOICES
    })
...
def assign_client(request):
    ...
    if client_id and user_id:
        ...
        ClientAssignment.objects.create(client=client, assigned_to=csm, assigned_by=user, is_active=True)
...
def client_detail(request, client_id):
    ...
    if perms['can_view_all']:
        ...
        client = get_object_or_404(Client, id=client_id, assignments__assigned_to=user, assignments__is_active=True)
    ...
    return render(request, 'dashboard/client_detail.html', {
        'client': client, 'stage_history': client.stage_history.select_related('changed_by'), 'form': form, 'perms': perms
    })
